...
def kline_profession(data_period: pd.DataFrame) -> Grid:
    stock_name = data_period.values[0][2]

    data_x = [x for x in data_period['日期']]
    data_v = [list(x) for x in data_period[['开盘价','收盘价','最低价','最高价']].values]
    data_volume = [x for x in data_period['成交量']]

    kline = (
        Kline()
        .add_xaxis(xaxis_data=data_x)
        .add_yaxis(
            series_name="{}日K".format(stock_name),
            y_axis=data_v,
            itemstyle_opts=opts.ItemStyleOpts(color="#ec0000", color0="#00da3c"),
        )
        .set_global_opts(
            title_opts=opts.TitleOpts(
                title="{}大盘走势图".format(stock_name),
